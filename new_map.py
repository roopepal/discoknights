...
class MapView(object):
	...
	def __init__(self, map_object):
		...
		self.draw_offset_x = self.width / 2
		# initialize map tile and range surfaces
		self.squares_image = pygame.Surface( (self.width, self.height) ).convert_alpha()
		self.range_image = pygame.Surface( (self.width, self.height) ).convert_alpha()
	...
	def render_movement_range(self):
		for coordinates in self.map.in_range:
			...
			self.range_image.blit( MOVE_RANGE_IND, (screen_x + self.draw_offset_x, screen_y) )
	...
	def render_attack_range(self):
		for coordinates in self.map.in_range:
			...
			screen_x, screen_y = map_to_screen(coordinates.x, coordinates.y)
			# blit movement range indicator
			self.range_image.blit( ATTACK_RANGE_IND, (screen_x + self.draw_offset_x, screen_y) )
	...
	def render_heal_range(self):
		for coordinates in self.map.in_range:
			...
			screen_x, screen_y = map_to_screen(coordinates.x, coordinates.y)
			# blit movement range indicator
			self.range_image.blit( HEAL_RANGE_IND, (screen_x + self.draw_offset_x, screen_y) )
